...
@Component
public class ExchangerateHostClient implements ProviderClient {
    private final WebClient wc;
    public ExchangerateHostClient(WebClient wc){ this.wc=wc; }

    @Override public String name(){ return "exchangerate.host"; }

    @Override
    public Mono<Map<CurrencyPair, Double>> fetch() {
        // Get USD base for GBP,ZAR then compute ZAR->GBP
        Mono<Map<String, Double>> usd = wc.get()
                .uri("https://api.exchangerate.host/latest?base=USD&symbols=GBP,ZAR")
        ...
        return usd.map(m -> {
            ...
            double zarGbp = usdGbp / usdZar; // (GBP per USD) / (ZAR per USD) = GBP per ZAR
